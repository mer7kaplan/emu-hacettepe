...
::: {style="text-align: justify;"}
Bu grafik, üç farklı senaryo boyunca her parça için zaman dağılımının gösteriyor. Bu grafikteki parça bir TLB'yi temsil eder. "Bekleme Zamanı", bir parçanın belirli bir işlem başlamadan önce sıraya alınarak geçirdiği süreyi temsil eder. Bu bekleme zamanı, bazı operatörlerin farklı alt gruplara ait montajlarda çalışması nedeniyle gerçekleşir. Dolayısıyla, bir parça geldiğinde belirli bir operatör uygun değilse, parça, operatör işlemi başlatana kadar sıraya alınır. Buna karşılık, "Katma Değer Zaman" metriği, bir parçanın bir montaj içinde geçirdiği gerçek işleme süresini temsil eder. Son olarak, "Toplam Zaman" basitçe bekleme süresi ve katma değer süresinin toplam süresidir. Grafikten anlaşıldığı üzere bir TLB'nin tamamlanması için geçen toplam süre yaklaşık olarak 39-40 saattir ve bu süre şirket tarafından sağlanan verilerle uyumludur.
:::
...
plot_total <- plot_ly(long_total_data, 
                      ...
                      text = ~Value, 
                      textposition = 'auto') %>%
  layout(title = "Toplam Zaman", 
         xaxis = list(title = ""), 
         yaxis = list(title = "Zaman"), 
         barmode = 'dodge',
...
plot_wait <- plot_ly(long_wait_data, 
                     ...
                     text = ~Value, 
                     textposition = 'auto') %>%
  layout(title = "Bekleme Zamanları", 
         xaxis = list(title = "", tickangle = 45), 
         yaxis = list(title = "Zaman"), 
         barmode = 'dodge',
         legend = list(title = list(text = 'Varyans Katsayısı')))
